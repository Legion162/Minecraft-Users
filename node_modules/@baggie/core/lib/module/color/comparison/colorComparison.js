...
export class ColorComparison {
  ...
  add(colors) {
    (Array.isArray(colors) ? colors : [colors]).forEach(color => {
      const parsedColor = ColorComparison.parseColorToLab(convertStringToRgb(color));
      this.haystack.push({
        ...parsedColor,
        source: color
      });
  ...
  compare(color) {
    if (this.haystack.length) {
      const needle = ColorComparison.parseColorToLab(convertStringToRgb(color));
      ...
      this.haystack.forEach(straw => {
        ...
        let strawPrimeHue = straw.b === 0 || strawPrimeA === 0 ? 0 : radiansToDegrees(Math.atan2(straw.b, strawPrimeA));
        strawPrimeHue += strawPrimeHue >= 0 ? 0 : 360;
        let needlePrimeHue = straw.b === 0 || strawPrimeA === 0 ? 0 : radiansToDegrees(Math.atan2(needle.b, needlePrimeA));
        ...
        const deltaHPrime = 2 * Math.sqrt(strawChromaPrime * needleChromaPrime) * Math.sin(degreesToRadians(deltahPrime) / 2);
        ...
        const RsubT = -2 * Math.sqrt(Math.pow(chromaBarPrime, 7) / (Math.pow(chromaBarPrime, 7) + Math.pow(25, 7))) * Math.sin(degreesToRadians(60 * Math.exp(-Math.pow((HBarPrime - 275) / 25, 2))));
