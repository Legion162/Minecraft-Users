 ...
 * ```
 */
export const getDistanceOnSphere = (from, to, diameter) => {
  const dLat = degreesToRadians(to.lat - from.lat);
  const dLon = degreesToRadians(to.lng - from.lng);
  const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(degreesToRadians(from.lat)) * Math.cos(degreesToRadians(to.lat)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
