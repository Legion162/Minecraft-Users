...
exports.getElementSiblings = void 0;
/**
 * Get all siblings of an element.
 */
const getElementSiblings = (element, options) => {
  const {
  ...
  if (includeOriginalElement && !fromElement && !untilElement) {
    ...
    return Array.from(parent.children).filter(element => !matchSelector || element.matches(matchSelector));
  }
  const siblings = [];

  // Look for siblings starting with this element
  let firstElement = fromElement === true ? element : fromElement || parent.firstElementChild;

  // Look for siblings ending with this element
  const lastElement = untilElement === true ? element : untilElement || parent.lastElementChild;
  do {
    if (firstElement) {
      ...
      if ((!sameAsOriginalElement || includeOriginalElement) && (!matchSelector || currentElement.matches(matchSelector))) {
...
exports.getElementSiblings = getElementSiblings;
//# sourceMappingURL=getElementSiblings.js.map