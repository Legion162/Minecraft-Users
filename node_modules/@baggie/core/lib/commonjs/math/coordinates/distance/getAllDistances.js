...
exports.getAllDistances = void 0;
 ...
 * ```ts
 * import { getAllDistances } from "@baggie/core";
 *
 ...
 * ```
 */
const getAllDistances = function (fromCoordinates) {
  for (var _len = arguments.length, toCoordinates = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    toCoordinates[_key - 1] = arguments[_key];
  }
  const baseCoordinates = fromCoordinates ? [fromCoordinates] : toCoordinates;
  const offset = fromCoordinates ? 0 : 1;
  const allDistances = [];
  baseCoordinates.forEach((set1, index1) => {
    toCoordinates.slice(index1 + offset).forEach((set2, index2) => {
      allDistances.push({
        distance: (0, _getDistance.getDistance)(set1, set2),
        coordinates: {
          from: fromCoordinates ? fromCoordinates : toCoordinates[index1],
          to: toCoordinates[index2 + index1 + offset]
        }
      });
    });
  });
  allDistances.sort((a, b) => a.distance > b.distance ? 1 : b.distance > a.distance ? -1 : 0);
  return allDistances;
};
exports.getAllDistances = getAllDistances;
//# sourceMappingURL=getAllDistances.js.map